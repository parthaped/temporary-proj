...
\midrule
Partha Pediredla & Project 1 Part 1 fixes, \texttt{datagen.c}, \texttt{project1.c} &
  Two-level tree, \texttt{read\_all}/\texttt{write\_all} loops, hidden-key
  aggregation, exit arg via pipe \\
Shiv Patel & Problem 1 (\texttt{project2\_p1.c}), Makefile &
  Worker \texttt{SIGTSTP} pause, leader decision logic, Rule 1/2/3 signal flow
  and termination prints \\
Philip Guan & Problem 2 Part 1 (\texttt{project2\_p2.c}), Q1 and Q2 write-ups &
  Four forked children, \texttt{sigaction}/\texttt{SA\_SIGINFO} handlers,
  child--parent signaling tests \\
Ved Patel & Problem 2 Q3 (\texttt{project2\_p2\_q3.c}), report and screenshots &
  Split signal masks, pending-queue inspection with \texttt{sigpending} and
  \texttt{sigtimedwait}/\texttt{sigwait}, LaTeX layout \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item \textbf{Overleaf sync:} the code appendices at the end use
        \verb|\lstinputlisting| to read those \texttt{.c} files from the project
        folder. They are not duplicated inside \texttt{Report.tex}. After you change
        any source on Linux or in Git, upload or sync the matching \texttt{.c} files
        into the same Overleaf project as \texttt{Report.tex}, then recompile so the
        PDF listings match the code you build with \texttt{make}.
\end{itemize}
